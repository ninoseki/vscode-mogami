...
describe('parseProject with Poetry', () => {
  ...
  it('should extract requirements from groups', () => {
    ...
    expect(result.dependencies).toEqual([[{ name: 'baz', type: 'ProjectName' }, [7, 0, 7, 14]]])
  })
  ...
  it('should extract legacy dev requirements', () => {
    ...
    expect(result.dependencies).toEqual([[{ name: 'bar', type: 'ProjectName' }, [7, 0, 7, 14]]])
  })
  ...
  it('should extract complex requirements', () => {
    ...
    expect(result.dependencies).toEqual([[{ name: 'black', type: 'ProjectName' }, [7, 0, 7, 129]]])
  })
  ...
  it('should extract expanded requirements', () => {
    ...
    expect(result.dependencies).toEqual([[{ name: 'black', type: 'ProjectName' }, [6, 0, 10, 55]]])
  })
...
describe('parseProject with PEP 631', () => {
  it('should extract basic requirements', () => {
    ...
    expect(result.dependencies).toEqual([
      ...
      [{ name: 'gidgethub', specifier: '>4.0.0', type: 'ProjectName' }, [3, 2, 3, 26]],
      [{ name: 'django', specifier: '>2.1', type: 'ProjectName' }, [4, 2, 4, 31]],
      [{ name: 'django', specifier: '>2.0', type: 'ProjectName' }, [5, 2, 5, 31]],
    ])
  })
  ...
  it('should extract requirements from extras', () => {
    ...
    expect(result.dependencies).toEqual([
      [{ name: 'httpx', type: 'ProjectName' }, [2, 2, 2, 9]],
      [{ name: 'gidgethub', specifier: '>4.0.0', type: 'ProjectName' }, [5, 7, 5, 31]],
    ])
  })
})
...
describe('parseProject with uv', () => {
  it('should extract requirements from constraint-dependencies', () => {
    ...
    expect(result.dependencies).toEqual([
      [{ name: 'grpcio', specifier: '<1.65', type: 'ProjectName' }, [1, 27, 1, 40]],
    ])
  ...
  it('should extract requirements from dev-dependencies', () => {
    ...
    expect(result.dependencies).toEqual([
      [{ name: 'ruff', specifier: '==0.5.0', type: 'ProjectName' }, [1, 20, 1, 33]],
    ])
  })
  ...
  it('should extract requirements from override-dependencies', () => {
    ...
    expect(result.dependencies).toEqual([
      [{ name: 'werkzeug', specifier: '==2.3.0', type: 'ProjectName' }, [1, 25, 1, 42]],
    ])
  })

  it('should extract source from tool.uv.index-url', () => {
    const document = makeTextDocumentLike(['[tool.uv]', "index-url = 'https://example.com'"])
...
describe('parseProject with git versioning', () => {
  ...
  it('should NOT extract requirements', () => {
    ...
    expect(result.dependencies).toEqual([
      [{ name: 'django', specifier: '==5.1.6', type: 'ProjectName' }, [2, 2, 2, 18]],
    ])
  })
})
